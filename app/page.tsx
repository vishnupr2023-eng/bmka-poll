...
export default function Home() {
  ...
  // Track contestant changes so we ONLY reset sliders when contestant actually switches
  const currentContestantRef = useRef<string | null>(null);
  ...
  // 4. Poll live stage session (SAFE POLLING - Won't wipe active slider inputs)
  const fetchSession = async () => {
    ...
    if (data?.value) {
      ...
      const activeCoupleId = live.current_couple_id;
      const isNewContestant = activeCoupleId !== currentContestantRef.current;

      if (activeCoupleId && live.status !== 'idle') {
        // Only fetch name if contestant changed or not yet loaded
        if (isNewContestant || !currentCoupleName) {
        ...
        // Check if voter already submitted for this specific couple
        if (voterToken) {
          ...
          if (vData) {
            ...
            // Show previously submitted locked score
            setRatings({
              outfit: vData.outfit,
            ...
            // ONLY reset sliders to 0 ONCE when the contestant actually changes!
            if (isNewContestant) {
      ...
      currentContestantRef.current = activeCoupleId;
    }
  };
